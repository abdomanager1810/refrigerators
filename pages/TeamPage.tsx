...
const TeamStatCard: React.FC<{ title: string; stats: { label: string; value: string | number }[] }> = ({ title, stats }) => (
    <div className="bg-white p-4 rounded-lg shadow-md flex-1">
        <h3 className="font-bold text-center text-gray-800 mb-3">{title}</h3>
...
const NavLinkItem: React.FC<{ icon: React.FC<any>, label: string, path: string }> = ({ icon: Icon, label, path }) => {
    ...
    return (
        <button onClick={() => path !== '#' && navigate(path)} className="flex items-center justify-between w-full p-4 bg-white rounded-lg shadow-md disabled:opacity-50" disabled={path === '#'}>
            <div className="flex items-center">
                <Icon className="w-6 h-6 text-indigo-500 mr-4" />
...
const TeamPage: React.FC = () => {
    ...
    const inviteLink = `${window.location.origin}${window.location.pathname}#/register?ref=${inviteCode}`;
    ...
    const navLinks = [
        ...
        { icon: PercentIcon, label: 'نسبة الاسترداد', path: '#' },
    ];
    ...
    return (
        <div className="bg-gray-100 min-h-screen">
            ...
            <main className="p-4 space-y-6">
                <div className="bg-white p-4 rounded-lg shadow-md text-center card-enter">
                    <p className="text-sm text-gray-500">إجمالي العمولة أمس</p>
                    <p className="text-3xl font-bold text-indigo-600 my-1">0.00</p>
                    <p className="text-xs text-indigo-400">قم بترقية المستوى لزيادة دخل العمولة</p>
                </div>
                ...
                <div className="card-enter" style={{animationDelay: '200ms'}}>
                    <h2 className="font-bold text-lg text-right mb-2">رابط الدعوة</h2>
                    <div className="bg-white p-3 rounded-lg shadow-md space-y-3">
                        <div className="flex items-center border border-gray-200 rounded-lg p-2">
                            <button onClick={() => copyToClipboard(inviteLink, 'تم نسخ رابط الدعوة!')} className="p-2 bg-indigo-500 text-white rounded-md mr-3 flex-shrink-0">
                        ...
                        <div className="flex items-center border border-gray-200 rounded-lg p-2">
                            ...
                            <span className="flex-grow text-right text-sm text-gray-500">نسخ رمز الدعوة</span>
                            <span className="font-mono tracking-widest text-indigo-600 ml-4">{inviteCode}</span>
                        </div>
                    </div>
                </div>
                ...
                <div className="bg-white p-4 rounded-lg shadow-md text-right card-enter" style={{animationDelay: '400ms'}}>
                    <h3 className="font-bold text-gray-800 mb-4">بيانات الترويج</h3>
                    <div className="grid grid-cols-2 gap-4 text-center">
                        <div>
                            <p className="text-lg font-bold text-indigo-600">{totalBonus}</p>
                            <p className="text-xs text-gray-500">إجمالي العمولة</p>
                        </div>
                         <div>
                         ...
                         <div>
                            <p className="text-lg font-bold text-indigo-600">{(currentUser?.team?.lv1?.length || 0) + (currentUser?.team?.lv2?.length || 0) + (currentUser?.team?.lv3?.length || 0)}</p>
                            <p className="text-xs text-gray-500">إجمالي الفريق</p>
                        </div>
                         <div>
                            <p className="text-lg font-bold text-indigo-600">{currentUser?.team?.lv1?.length || 0}</p>
                            <p className="text-xs text-gray-500">المرؤوس المباشر</p>
                        </div>
                    </div>
                </div>
            </main>
