...
const CustomerServicePage: React.FC = () => {
    ...
    const serviceLinks = config.customerServiceLinks;
                ...
                <main className="p-4 space-y-3 -mt-4 relative z-20">
                    {serviceLinks.map((item) => (
                        <a 
                            ...
                            className="bg-gray-700 p-4 rounded-lg shadow flex justify-between items-center w-full"
                        >
                             <div className="flex items-center">
                                <div className="w-10 h-10 bg-blue-500/20 rounded-full flex items-center justify-center ml-3">
                                     <SendIcon className="w-5 h-5 text-blue-400" />
                                </div>
                                <span className="font-semibold text-gray-200">{item.name}</span>
                            </div>
                            <ChevronLeftIcon className="w-5 h-5 text-gray-500 transform scale-x-[-1]" />
                        </a>
                    ))}

                    <div className="bg-gray-700 p-4 rounded-lg shadow text-right text-gray-300 text-sm leading-relaxed space-y-3">
